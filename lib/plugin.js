...
function shouldBeLazy(wpModule) {
  const { dependencies, resourceResolveData } = wpModule;
  ...
  return dependencies.some(dep => {
    ...
    // ignore html entry, because our laoder only generate js
    return isEntryDep(dep) && !resourceResolveData.path.endsWith('.html');
  });
}
...
class LazyCompilePlugin {
  ...
  _collectLazyModules(wpModule) {
    const { userRequest } = wpModule;

    // const { inspect } = require('util');
    // if (i++ < 10) {
    //   console.log('wpModule', inspect(wpModule, { depth: 1 }));
    // }

    if (shouldBeLazy(wpModule) && !this._lazyModules.has(userRequest)) {
      this._lazyModules.set(userRequest, {
